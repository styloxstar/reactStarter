...
export const FormSplit = () => (
  <div className="w-full max-w-4xl bg-[hsl(var(--background))] dark:bg-gray-900 rounded-2xl shadow-xl overflow-hidden grid grid-cols-1 md:grid-cols-2 border border-[hsl(var(--border))]">
    {/* Image Side */}
    ...
    <div className="p-10 flex flex-col justify-center">
      <h2 className="text-2xl font-bold text-[hsl(var(--foreground))] mb-6">Create Account</h2>
      <form className="space-y-4">
        <input type="text" placeholder="Full Name" className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg focus:outline-none focus:border-indigo-500 focus:bg-white dark:focus:bg-gray-900 transition-colors text-[hsl(var(--foreground))]" />
        <input type="email" placeholder="Email Address" className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg focus:outline-none focus:border-indigo-500 focus:bg-white dark:focus:bg-gray-900 transition-colors text-[hsl(var(--foreground))]" />
        <input type="password" placeholder="Password" className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg focus:outline-none focus:border-indigo-500 focus:bg-white dark:focus:bg-gray-900 transition-colors text-[hsl(var(--foreground))]" />
...
export const FormMinimal = () => (
  <div className="w-full max-w-md p-8 glass-card border border-[hsl(var(--border))]">
    <h2 className="text-3xl font-light text-[hsl(var(--foreground))] mb-8 tracking-wide">Contact Us.</h2>
    <form className="space-y-8">
      <div className="group">
        <input type="text" required className="w-full py-2 bg-transparent border-b border-[hsl(var(--border))] focus:border-[hsl(var(--foreground))] text-[hsl(var(--foreground))] outline-none transition-colors peer" />
        <label className="block text-sm text-[hsl(var(--foreground))] opacity-40 transition-all peer-focus:-translate-y-6 peer-focus:text-xs peer-focus:text-[hsl(var(--foreground))] peer-valid:-translate-y-6 peer-valid:text-xs">Your Name</label>
      </div>
      <div className="group">
        <input type="email" required className="w-full py-2 bg-transparent border-b border-[hsl(var(--border))] focus:border-[hsl(var(--foreground))] text-[hsl(var(--foreground))] outline-none transition-colors peer" />
        <label className="block text-sm text-[hsl(var(--foreground))] opacity-40 transition-all peer-focus:-translate-y-6 peer-focus:text-xs peer-focus:text-[hsl(var(--foreground))] peer-valid:-translate-y-6 peer-valid:text-xs">Email Address</label>
      </div>
      <div className="group">
        <textarea required rows="1" className="w-full py-2 bg-transparent border-b border-[hsl(var(--border))] focus:border-[hsl(var(--foreground))] text-[hsl(var(--foreground))] outline-none transition-colors peer resize-none"></textarea>
        <label className="block text-sm text-[hsl(var(--foreground))] opacity-40 transition-all peer-focus:-translate-y-6 peer-focus:text-xs peer-focus:text-[hsl(var(--foreground))] peer-valid:-translate-y-6 peer-valid:text-xs">Message</label>
      </div>
      <button className="px-8 py-3 border border-[hsl(var(--foreground))] text-[hsl(var(--foreground))] font-medium hover:bg-[hsl(var(--foreground))] hover:text-[hsl(var(--background))] transition-colors uppercase text-sm tracking-widest">
        Send Message
      </button>
    </form>
  </div>
...
export const FormFloating = () => (
  <div className="w-full max-w-md p-8 glass-card border border-[hsl(var(--border))] rounded-xl shadow-lg">
    <h2 className="text-2xl font-bold text-[hsl(var(--foreground))] mb-6">Create Account</h2>
    <form className="space-y-5">
      {['First Name', 'Last Name', 'Email'].map((label) => (
        <div key={label} className="relative">
          <input type="text" id={label} className="block px-2.5 pb-2.5 pt-4 w-full text-sm text-[hsl(var(--foreground))] bg-transparent rounded-lg border border-[hsl(var(--border))] appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " />
          <label htmlFor={label} className="absolute text-sm text-[hsl(var(--foreground))] opacity-60 duration-300 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] bg-[hsl(var(--card))] px-2 peer-focus:px-2 peer-focus:text-blue-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto start-1 pointer-events-none">
            {label}
          </label>
        </div>
      ))}
...
export const FormPayment = () => (
  <div className="w-full max-w-md p-6 glass-card border border-[hsl(var(--border))] rounded-xl shadow-2xl">
    <div className="flex justify-between items-center mb-6">
      <h2 className="text-xl font-bold text-[hsl(var(--foreground))]">Payment Details</h2>
      <div className="flex gap-2">
         <div className="w-8 h-5 bg-[hsl(var(--foreground))] opacity-10 rounded"></div>
         <div className="w-8 h-5 bg-[hsl(var(--foreground))] opacity-10 rounded"></div>
      </div>
    </div>
    <form className="space-y-4">
      <div>
        <label className="text-xs font-bold text-[hsl(var(--foreground))] opacity-40 uppercase tracking-wide">Card Number</label>
        <div className="relative mt-1">
          <input type="text" placeholder="0000 0000 0000 0000" className="w-full pl-10 pr-4 py-3 bg-[hsl(var(--card))] border border-[hsl(var(--border))] rounded-lg focus:bg-[hsl(var(--background))] focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-[hsl(var(--foreground))]" />
          <svg className="w-5 h-5 text-[hsl(var(--foreground))] opacity-40 absolute left-3 top-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" /></svg>
        </div>
      </div>
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="text-xs font-bold text-[hsl(var(--foreground))] opacity-40 uppercase tracking-wide">Expiry</label>
          <input type="text" placeholder="MM / YY" className="w-full mt-1 px-4 py-3 bg-[hsl(var(--card))] border border-[hsl(var(--border))] rounded-lg focus:bg-[hsl(var(--background))] focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-[hsl(var(--foreground))]" />
        </div>
        <div>
          <label className="text-xs font-bold text-[hsl(var(--foreground))] opacity-40 uppercase tracking-wide">CVC</label>
          <input type="text" placeholder="123" className="w-full mt-1 px-4 py-3 bg-[hsl(var(--card))] border border-[hsl(var(--border))] rounded-lg focus:bg-[hsl(var(--background))] focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-[hsl(var(--foreground))]" />
        </div>
      </div>
      <button className="w-full py-4 mt-2 bg-slate-900 dark:bg-slate-100 dark:text-slate-900 text-white font-bold rounded-lg hover:opacity-90 transition-colors flex justify-center items-center gap-2">
...
export const FormSearch = () => (
  <div className="w-full max-w-lg p-1 rounded-2xl bg-gradient-to-r from-rose-400 via-fuchsia-500 to-indigo-500">
    <div className="bg-[hsl(var(--background))] rounded-xl p-2 flex items-center shadow-lg border border-[hsl(var(--border))]">
      <div className="pl-4 text-gray-400">
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
      </div>
      <input type="text" placeholder="Search the cosmos..." className="w-full p-4 text-[hsl(var(--foreground))] bg-transparent outline-none text-lg" />
      <button className="px-6 py-2 bg-[hsl(var(--primary))] text-white font-semibold rounded-lg hover:opacity-90 transition-colors">
        Search
      </button>
    </div>
  </div>
...
const Forms = () => {
  return (
    <div className="container bg-transparent p-6 md:p-12 font-sans overflow-y-auto">
      <h1 className="text-4xl font-bold text-center text-[hsl(var(--foreground))] mb-4">Modern Form Layouts</h1>
      <p className="text-center text-[hsl(var(--foreground))] opacity-60 mb-16 max-w-2xl mx-auto">A collection of responsive, production-ready form components using React & Tailwind CSS.</p>
...
.glass-form button { width: 100%; padding: 0.75rem; background: linear-gradient(90deg, #ec4899, #7c3aed); color: #fff; font-weight: 700; border-radius: 0.5rem; border: none; cursor: pointer; }`} />
        ...
        {/* 2. Split */}
        <div className="w-full flex flex-col items-center glass-card p-6 rounded-xl relative group/form">
          <div className="absolute top-2 right-2 z-40 opacity-0 group-hover/form:opacity-100 transition-opacity">
